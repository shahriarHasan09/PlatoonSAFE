...
void PlatoonsPlusHumanTraffic::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        humanInsertTime = SimTime(par("humanInsertTime").doubleValue());
        ...
        insertHumanMessage = new cMessage("");
        scheduleAt(humanInsertTime, insertHumanMessage);
    }
}
...
void PlatoonsPlusHumanTraffic::handleSelfMsg(cMessage* msg)
{
    ...
    else if (msg == insertHumanMessage) {
        insertHumans();
    }
}
...
PlatoonsPlusHumanTraffic::~PlatoonsPlusHumanTraffic()
{
    ...
    cancelAndDelete(insertHumanMessage);
    insertHumanMessage = nullptr;
}
